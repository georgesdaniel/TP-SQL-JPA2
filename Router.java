...
public class Router implements SparkApplication {
	...
	public void init() {
		...
		get("/exemple1", (request, response) -> {
			...
			// Tp conflit avec commit
						           
			entityManager.getTransaction().begin();
			Maire testmaire = entityManager.find(Maire.class, 46L) ;
			testmaire.setNom("zouba");
			//entityManager.persist(testmaire);
			
			entityManager.getTransaction().commit();
			
			
			//Suppression d'un Maire
			
			//Maire supMaire = entityManager.find(Maire.class, 43L);
			
			//entityManager.getTransaction().begin();
			//entityManager.remove(supMaire);
			//entityManager.getTransaction().commit();
			...
			//TypedQuery<Maire> query2 = entityManager.createQuery ("from Maire", Maire.class);
			//attributes.put("objets",query2.getResultList());
